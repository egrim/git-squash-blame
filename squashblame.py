...
def squash_blame():
    ...
    for blob in head_commit.tree.traverse():
        if isinstance(blob, Blob):
            ...
            for author, authors_line_numbers in author_lines.items():
                ...
                for line_number, line in enumerate(mutated_blob_bytes.splitlines(keepends=True), start=1):
                ...
                authors_fixed_blob = make_blob(repo, authors_fixed_blob_bytes, blob.mode, blob.path)
                author_blob_lists[author].append(authors_fixed_blob)

    base_index = IndexFile.from_tree(repo, head_commit)
    base_index.add(mutated_blobs)
    ...
    for author, author_blobs in author_blob_lists.items():
        ...
        index.add(author_blobs)
        author_commits.append(
            index.commit(message=f"Squash while preserving git-blame for {author.name}",
    ...
    final_commit = final_index.commit(message="Merge commits that preserve git-blame",
